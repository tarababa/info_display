...
def eskom_get_loadshedding_schedule(province,suburb,suburbId,suburbTot, lsstatus, powerStatus, forecast):
  ...
  logger.debug('start')
  #set the url
  if forecast.stage != None and lsstatus.isdigit() and forecast.stage.isdigit() and int(forecast.stage)>=int(STATUS2STAGE[lsstatus][0]):
...
def eskom_twitter(lsstatus):
  ...
  r=twitter.search(q='from:Eskom_SA -stopped -cancelled -restoration -restored)')
